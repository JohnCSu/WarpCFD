...
from warp_cfd.FV.kernels import equation_kernels
class Equation():
    def __init__(self,fvm:FVM,fields:str| list[str],solver = linear.bicgstab) -> None:
        ...
        wp.launch(kernel=equation_kernels.calculate_BSR_matrix_indices,dim = [fvm.num_cells,fvm.faces_per_cell+1,self.num_outputs],inputs = [self.COO_array.rows,
                                                                                                                  self.COO_array.cols,
    ...
    def replace_row(self,row_id:int | wp.array,rhs_value: float | wp.array):
        ...
        wp.launch(equation_kernels.replace_row_kernel,dim = row_id.shape, inputs=[self.A.offsets,self.A.columns,self.A.values,self.rhs,row_id,value])
